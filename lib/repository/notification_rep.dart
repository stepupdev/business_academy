...
class NotificationRep {
  ...
  Future markNotification(String id, BuildContext context) async {
    final response = await manager.postAPICallWithHeader(context, "${ApiUrl.notification}/$id/mark-read", {}, {
      "Authorization": "Bearer ${Get.find<AuthService>().currentUser.value.result!.token}",
      "Accept": "application/json",
    });
    debugPrint("response: $response");
  ...
  Future markReadUnreadNotification(String id, BuildContext context) async {
    final response = await manager.postAPICallWithHeader(context, "${ApiUrl.notification}/$id/mark-read-unread", {}, {
      "Authorization": "Bearer ${Get.find<AuthService>().currentUser.value.result!.token}",
      "Accept": "application/json",
    });
    debugPrint("response: $response");
  ...
  Future markAllNotification(BuildContext context) async {
    final response = await manager.postAPICallWithHeader(context, "${ApiUrl.notification}/mark-all-read", {}, {
      "Authorization": "Bearer ${Get.find<AuthService>().currentUser.value.result!.token}",
      "Accept": "application/json",
    });
    debugPrint("response: $response");
